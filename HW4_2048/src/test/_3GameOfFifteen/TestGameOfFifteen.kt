...
import task.newGameOfFifteen
import util.TestGame

class TestGameOfFifteen : TestGame() {
    ...
    @Test
    fun testHasWon() {
        Assert.assertFalse("Initial permutation should be random, but with the right parity. " +
                "You win when the numbers are in order", game.hasWon())
    }
